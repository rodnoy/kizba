...
Phase D.4 verification passed: mounted `GitConflictBanner` as a sheet in `KizbaApp` using `state.router.isGitConflictBannerPresented` and the existing `state.gitStatusModel` instance, with an `openTerminalAction` that opens the store path via `NSWorkspace`. Added lightweight `GitConflictBannerMountTests` (2 executed, 0 failed) covering router-flag toggling and non-crashing sheet-content construction when a model is present. `xcodebuild test -only-testing:KizbaTests/GitConflictBannerMountTests` succeeded, `xcodebuild build -scheme Kizba -project Kizba.xcodeproj -destination 'platform=macOS'` succeeded, and both grep bans (`\bas!\b` and `Logger.*stdin|print\(.*stdin`) returned no matches.
