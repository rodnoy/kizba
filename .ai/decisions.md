...
## 2026-05-17 — MVP 5 (Search / Favorites / Recents / Menu-bar)

- **`LiveSearchEngine`** — pure in-memory ranker. Pure-actor, no persistence, no index file; recomputed per query from the in-memory entry list returned by `PassManaging.listEntries()`. Base score is one of fixed buckets `1.0 / 0.9 / 0.8 / 0.7 / 0.6` (exact-match / title-prefix / path-prefix / title-contains / path-contains). When a `SearchContext` is passed, score is boosted `+0.05` for entries in `favoritePaths` and `+0.03` for entries in `recentPaths`; the boosts are summed and the final score is capped at `1.0`. Tie-breaker is case-insensitive ascending path order.
- **`FavoritesStoring`** — actor-based protocol. Production impl `UserDefaultsFavoritesStore` is a `public actor`, backed by `UserDefaults` under key `kizba.favorites` (storing `[String]` of entry paths). Change notifications via `nonisolated var favoritesChanged: AsyncStream<Void>` (multi-subscriber pattern: per-subscriber `UUID` registered in an actor-isolated continuations dict, auto-unregistered via `onTermination`).
- **`RecentEntriesStoring`** — actor-based protocol. Production impl `UserDefaultsRecentEntriesStore` is a `public actor`, backed by `UserDefaults` under key `kizba.recentEntries` (storing `[String]` of entry paths, newest first). Default `maxCount = 20`, clamped to `max(1, maxCount)`. `record(_:)` dedup-moves-to-front (removes any prior occurrence then inserts at index 0), trims to `maxCount`. Change notifications via `nonisolated var recentsChanged: AsyncStream<Void>` (same multi-subscriber pattern as favorites).
- **`StatusItemController`** — `@MainActor`, owns `NSStatusItem` (`NSStatusItem.variableLength`, SF Symbol `key.fill` as the button image, accessibility description "Kizba menu bar") + `NSPopover` (`behavior = .transient`, content sized 360×420). SwiftUI content hosted via `NSHostingView` set as the popover's `contentViewController.view`. Visibility gated by `SettingsKeys.showInMenuBar` (`defaultShowInMenuBar = true`). Reactive to `UserDefaults.didChangeNotification` so toggling the setting shows/hides the status item live without restart.
- **D.4 global hotkey** — deferred. `NSEvent.addGlobalMonitorForEvents` requires Accessibility permission, and the system prompt UX is intrusive (and persistent: the app appears under Privacy & Security → Accessibility forever after). Revisit only when there is concrete user demand; meanwhile the menu-bar status item click + ⌘K in-app overlay cover the same need.

## 2026-05-17 — MVP6.D.1 — Biometric toggle semantics

- Enabling Touch ID does NOT require a biometric prompt. Matches macOS pattern (FileVault, Touch ID settings panel). Future audits may add a second flow.
- Disabling Touch ID REQUIRES successful `LAContext` authentication. On cancel/fail — persisted value is unchanged; UI surfaces a banner.
- A previously-enabled but now-unavailable biometric (hardware change) does NOT auto-flip the persisted value. The UI in SecurityTab (D.2) reflects availability live; the setting remains `true` until the user manually disables it (which is no-op without working biometric, an edge case).
