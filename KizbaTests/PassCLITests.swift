...
// MARK: - FakeShellRunner

/// Test double for `ShellCommandRunning`. Captures the invocation it
/// receives and replays a programmed response after an optional delay.
///
/// Concurrency: `Sendable` via an internal `NSLock`. State is mutated
/// from the runner task; assertions read it after `await`-ing the
/// `PassCLI.show(_:)` call so memory ordering is guaranteed.
final class FakeShellRunner: ShellCommandRunning, @unchecked Sendable {
    ...
    enum Response: Sendable {
        /// Return a `ShellResult` after `delay`.
        case success(exitCode: Int32, stdout: Data, stderr: Data)
        /// Throw a `PassError` after `delay`.
        case throwing(error: PassError, after: Duration)
    }
    ...
    init(response: Response, delay: Duration = .zero) {
    ...
    func run(
        ...
        // Honour Task cancellation by translating into `.cancelled` —
        // mirrors `ProcessShellRunner`'s production contract so tests
        // exercise the same code path in `PassCLI`.
        let activeDelay: Duration
        switch response {
        case .success:
            activeDelay = delay
        case .throwing(_, let d):
            activeDelay = d
        }
        ...
        switch response {
        case .success(let code, let out, let err):
